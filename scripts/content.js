...
//Popup functions and other utilities
...
//Header summary creation and  Styling
function headerDisplay(text, color) {
  ...
  title.style.fontFamily = "Trebuchet MS,Lucida Grande,Lucida Sans Unicode,Lucida Sans,Tahoma,sans-serif";
  title.style.fontSize = "22px";
  title.style.fontWeight = "bold";
  ...
  tldr.style.fontFamily = "Trebuchet MS,Lucida Grande,Lucida Sans Unicode,Lucida Sans,Tahoma,sans-serif";
...
async function summarize(text, options) {
  chrome.storage.sync.get(['apiKey', 'options'], async (data) => {
    ...
    if (!apiKey) {
      headerDisplay("Please set an API key in WebPeek > Options. You can get one from https://dashboard.cohere.ai/api-keys", "#d18ee2");
      return;
    }
    ...
    try {
      ...
      if (summaryResponse.summary === undefined) {
        ...
        if (targetLanguage === 'en') {
          headerDisplay(summary, options.displayColor);
        } else {
          ...
          headerDisplay(translatedSummary.translation_data.translation, options.displayColor);
        }
      }
    } catch (error) {
      headerDisplay("An error occurred: " + error.message, options.displayColor);
    }
  });
}
...
async function tts(summary) {
  const apiKey = '30c449f598msh3e861e1c2371d0fp181ac7jsn98e3ed20cbc1';

  // Set up the request to send to the TTS endpoint
  const ttsOptions = {
    method: 'POST',
    headers: {
      'content-type': 'application/x-www-form-urlencoded',
  ...
  try {
    const ttsResponse = await fetch('https://voicerss-text-to-speech.p.rapidapi.com/?key=' + apiKey, ttsOptions);
    ...
    // audio.play();
      ...
      console.log("An error occurred while generating TTS:", error.message);
  }
}
...
function getVisibleText() {
    // Using jQuery selectors, try to find the page's main body of content, often in a content or main element.
    ...
    for (let i = 0, max = allTags.length; i < max; i++) {
        ...
        if (!isHidden(elem)) {
            ...
            if (nodeValue) {
                nChars += nodeValue.length + 1; // for newline
                if (nChars < charLimit) {
...
document.body.addEventListener("mouseover", async (e) => {
  chrome.storage.sync.get('apiKey', async (data) => {
    const apiKey = data.apiKey;
    if (!apiKey) {
      // Display a message to the user to input the API key
    ...
    if (foundTag && foundTag !== previousTag) {
      ...
      try {
        ...
        // Fetch Open Graph tags using the CDN URL
        const ogDataResponse = await fetch(`${cdnURL}${foundTag.href}`, { signal: abortController.signal });
        
        // Check if the request was aborted intentionally by the user
...
function main() {
  chrome.storage.sync.get(['apiKey', 'options', 'displayColor', 'fontSize'], data => {
      const apiKey = data.apiKey;
      if (!apiKey) {
          headerDisplay("Please set an API key in WebPeek > Options. You can get one from https://dashboard.cohere.ai/api-keys", "#d18ee2");
          return;
      ...
      summarize(truncatedVisibleText, options);
  });
}
